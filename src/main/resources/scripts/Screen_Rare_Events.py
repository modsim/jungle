...
from de.fzj.jungle.trackmate.action import FluorescenceMaximumScreener
...
#	action.execute(trackmate, phyloFile)
	...
	# Instantiate action
	#-------------------
	action = FluorescenceMaximumScreener()
	action.execute(trackmate)

	return action.getMaximumValues()
...
#		IJ.run(imp, "To ROI Manager", "");
		...
		# Execute trackmate
		maximumValues = executeTrackmate(path)
...
#		RoiManager.getInstance().runCommand("Delete")
		...
		for val in maximumValues:
			fileResult.append(val)

		#-------------------
		# Write to results
		#-------------------
		with open(os.path.join(root, 'results_maxima.csv'), 'a') as resultsFile:
